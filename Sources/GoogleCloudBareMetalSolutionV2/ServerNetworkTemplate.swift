...
/// Network template.
public struct ServerNetworkTemplate: Codable, Equatable, GoogleWKT._AnyPackable,
  Sendable
{
  ...
  ///
  /// ```
  /// let value = ServerNetworkTemplate().with { $0.name = ... }
  /// ```
    ...
    Sendable
  {
    ...
    /// Interface type.
    public var type: ServerNetworkTemplate.LogicalInterface.InterfaceType = ServerNetworkTemplate
      .LogicalInterface.InterfaceType()
    ...
    ///
    /// ```
    /// let value = LogicalInterface().with { $0.name = ... }
    /// ```
    public func with(_ config: (inout Self) throws -> Swift.Void) rethrows -> Self {
      var copy = self
      try config(&copy)
  ...
  public static var _anyTypeUrl: Swift.String {
    return "type.googleapis.com/google.cloud.baremetalsolution.v2.ServerNetworkTemplate"
  }
  public init(fromAny any: GoogleWKT.`Any`) throws {
    self = try GoogleWKT._slowAnyDeserialize(Self.self, from: any)
  }
